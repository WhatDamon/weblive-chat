...
| 可见但久无操作也释放流 | 无鼠标/键盘/滚动 5 分钟后 abort，转 30s 低频轮询；任一操作立即恢复常连（内置页已实现，`window.WL` 可覆盖阈值） | 忘记关的标签页从 2 GB-hr/小时降至约 0.03–0.1 GB-hr/小时（省 20–60 倍） |
...
| 内置页 `s-maxage=300` | 边缘缓存页面响应；API 一律 `no-store`（`/api/meta` 含调用方 IP） | 重复页面浏览不计实例时间 |

> **可支撑规模**：全额度 = 每天 ≈6 小时实例时间 ≈ **36 人 × 10 分钟「可见且活跃」/天**；因 Hobby 超限是暂停功能 30 天，建议按一半规划（≈18 人 × 10 分钟/天）。隐藏与久无操作时间已被上述措施降到近 0。前端侧的落地清单见 `docs/quota.md`。
>
> 量级估算：常连固定 **2 GB-hr/小时**；30s 低频轮询按每次请求占实例 0.5–1.5s 计约 **0.03–0.1 GB-hr/小时**（冷启动越多越靠上限）。
>
> 已评估而**故意不做**：懒加载未使用的驱动（实测冷启动总模块成本仅 ~8–10 ms，收益 ≈0.02 CPU-h/月）；把轮询/presence 节拍做成环境变量（对约束无影响，只会膨胀 `.env`）；per-instance 共享轮询器（能减少 DB 读，但 DB 读不是瓶颈，不值得共享可变状态的复杂度）。
