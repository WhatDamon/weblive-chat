...
describe("rateCheck", () => {
  test("未超限 allowed；第 limit+1 次拒绝并给 retry_after_ms", async () => {
    ...
    expect(await rateCheck(repo as any, "msg", "9.9.9.9", 2, now)).toMatchObject({ allowed: true, count: 1 });
    expect(await rateCheck(repo as any, "msg", "9.9.9.9", 2, now)).toMatchObject({ allowed: true, count: 2 });
